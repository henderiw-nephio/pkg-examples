...
var (
	...
	ipAllocationStatusPrefix = []string{"status", "ipAllocationStatus", "prefix"}
	ipAllocationStatusGateway = []string{"status", "ipAllocationStatus", "gateway"}
)
...
// SetIPAllocationStatusPrefix sets the prefix of the ipAllocatonStatus in the status
func (r *Interface) SetIPAllocationStatusPrefix(s string) error {
	return r.SetNestedString(s, ipAllocationStatusPrefix...)
}

// SetIPAllocationStatusGateway sets the gateway of the ipAllocatonStatus in the status
func (r *Interface) SetIPAllocationStatusGateway(s string) error {
	return r.SetNestedString(s, ipAllocationStatusGateway...)
}
...
func (r *Interface) DeleteIPAllocationStatusPrefix() (bool, error) {
	return r.RemoveNestedField(ipAllocationStatusPrefix...)
}

func (r *Interface) DeleteIPAllocationStatusGateway() (bool, error) {
	return r.RemoveNestedField(ipAllocationStatusGateway...)
}

func (r *Interface) SetIPAllocationStatus(ipallocStatus *ipamv1alpha1.IPAllocationStatus) error {
	if ipallocStatus == nil {
		return nil
	}
	if ipallocStatus.AllocatedPrefix != "" {
		if err := r.SetIPAllocationStatusPrefix(ipallocStatus.AllocatedPrefix); err != nil {
			return err
		}
	} else {
		if _, err := r.DeleteIPAllocationStatusPrefix(); err != nil {
			return err
		}
	}
	if ipallocStatus.Gateway != "" {
		if err := r.SetIPAllocationStatusGateway(ipallocStatus.Gateway); err != nil {
			return err
		}
	} else {
		if _, err := r.DeleteIPAllocationStatusGateway(); err != nil {
			return err
		}
	}
	return nil
